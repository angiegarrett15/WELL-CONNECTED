...
const initialResources = [
  {
    ...
    category: 'Housing',
    description: 'Central intake placeholder for emergency housing, shelter referral, and short-term stabilization needs.',
    contact: 'housing@example.org',
    phone: '555-0101',
  ...
  },
  {
    ...
    description: 'Food box and household essentials distribution. Replace with verified local schedule and eligibility details.',
  ...
  {
    id: 3,
    title: 'Shade Tree Mechanic Referral',
    category: 'Community Skills',
    description: 'Community-level service placeholder for low-cost vehicle repair referrals after admin verification.',
    contact: 'mechanic@example.org',
    phone: '555-0103',
  ...
  {
    id: 4,
    title: 'Plain-Language Research Briefs',
    category: 'Research Hub',
    description: 'Research findings translated into short community-facing explanations with practical meaning and limitations.',
...
const initialPolicies = [
  {
    id: 1,
    title: 'Beneficial Policy Example',
    type: 'Beneficial',
    summary: 'Policies that increase access, reduce stigma, or improve service connection belong here.',
    status: 'approved'
  },
  {
    id: 2,
    title: 'Harmful Policy Example',
    type: 'Harmful',
    summary: 'Policies or practices that create barriers, stigma, unnecessary punishment, or exclusion belong here.',
    status: 'approved'
  },
  {
    id: 3,
    title: 'Missing Policy Example',
    type: 'Missing',
    summary: 'Community needs with no adequate policy, program, or referral pathway belong here.',
    status: 'pending'
  }
];
  ...
  return (
    <article className='resource-card'>
      <div className='resource-topline'>
        ...
        <span className='category-chip'><Icon size={15} /> {item.category}</span>
      </div>
      <h3>{item.title}</h3>
      <p>{item.description}</p>
      <div className='meta-grid'>
        {item.location && <span><MapPin size={15} /> {item.location}</span>}
        {item.phone && <span><Phone size={15} /> {item.phone}</span>}
...
function PolicyBoard({ policies, setPolicies, isAdmin }) {
  ...
  function addPolicy(event) {
    ...
    setPolicies((items) => [{ ...draft, id: Date.now(), status: isAdmin ? 'approved' : 'pending' }, ...items]);
    setDraft({ title: '', type: 'Beneficial', summary: '' });
  }

  function approve(id) {
    setPolicies((items) => items.map((item) => item.id === id ? { ...item, status: 'approved' } : item));
  }
    ...
    <section className='panel'>
      <div className='section-heading'>
        ...
        <div>
          ...
          <p>Sort policies into beneficial, harmful, and missing categories for review and community education.</p>
        </div>
      </div>
      <form className='inline-form' onSubmit={addPolicy}>
        <input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} placeholder='Policy or barrier title' />
        <select value={draft.type} onChange={(e) => setDraft({ ...draft, type: e.target.value })}>
          <option>Beneficial</option>
          <option>Harmful</option>
          <option>Missing</option>
        </select>
        <input value={draft.summary} onChange={(e) => setDraft({ ...draft, summary: e.target.value })} placeholder='Brief description' />
          ...
          <div className='policy-column' key={type}>
            ...
            {visiblePolicies.filter((policy) => policy.type === type).map((policy) => (
              <div className='policy-card' key={policy.id}>
                ...
                <h4>{policy.title}</h4>
                <p>{policy.summary}</p>
                {isAdmin && <div className='actions'>{policy.status === 'pending' && <button onClick={() => approve(policy.id)}>Approve</button>}<button className='ghost danger-text' onClick={() => remove(policy.id)}>Delete</button></div>}
              </div>
            ))}
          </div>
        ))}
...
function App() {
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((item) => {
      const participantCanSee = isAdmin || item.status === 'approved';
      const categoryMatch = activeCategory === 'All' || item.category === activeCategory;
      const pendingMatch = !showPendingOnly || item.status === 'pending';
      const haystack = `${item.title} ${item.category} ${item.description} ${item.location} ${item.tags.join(' ')}`.toLowerCase();
      const searchMatch = haystack.includes(searchTerm.toLowerCase());
      return participantCanSee && categoryMatch && pendingMatch && searchMatch;
    });
    ...
    <div className='app-shell'>
      ...
      <main className='workspace'>
        <header className='hero'>
          <div>
            <span className='eyebrow'><Megaphone size={16} /> Community-informed public health infrastructure</span>
